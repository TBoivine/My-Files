...
if (isset($_GET['id'])) {
    ...
    $delete_subscribers_query = "DELETE FROM service_plans WHERE service_id = $service_id";
    if ($conn->query($delete_subscribers_query) === TRUE) {
        // If subscribers are deleted successfully, delete the service
        $delete_service_query = "DELETE FROM service_plans WHERE service_id = $service_id";
